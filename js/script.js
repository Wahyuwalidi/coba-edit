...
function makeTodo(todoObject) {
  ...
  inputJudul.innerText = title;

  const inputAuthor = document.createElement('h3');
  inputAuthor.innerText = author;

  const inputYear = document.createElement('p');
  inputYear.innerText = year;

  const textContainer = document.createElement('div');
  textContainer.classList.add('inner');
  textContainer.append(inputJudul, inputAuthor, inputYear);
...
function addTodo() {
  ...
  const inputYear = document.getElementById('month').value;
  // const inputRead = document.getElementById('checkbox').value;

  const generatedID = generateId();
  const todoObject = generateTodoObject(generatedID, inputJudul, inputAuthor, inputYear, false);
